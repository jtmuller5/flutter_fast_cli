...
class BasicObserver extends NavigatorObserver {
  ...
  void didPush(Route<dynamic>? route, Route<dynamic>? previousRoute) {
    ...
    if (route?.settings.name != null) {
      analyticsService.logEvent('view_screen', eventProperties: {'screen_name': route?.settings.name, 'type': 'push'});
    }
  }
  ...
  void didReplace({Route<dynamic>? newRoute, Route<dynamic>? oldRoute}) {
    ...
    analyticsService.logEvent('view_screen', eventProperties: {'screen_name': newRoute?.settings.name, 'type': 'replace'});
  }

  @override
  void didRemove(Route<dynamic>? route, Route<dynamic>? previousRoute) {
    debugPrint('Basic observer - Remove: (${previousRoute?.settings.name}) <== (${route?.settings.name})');
    analyticsService.logEvent('view_screen', eventProperties: {'screen_name': previousRoute?.settings.name, 'type': 'remove'});
  }

  @override
  void didPop(Route<dynamic>? route, Route<dynamic>? previousRoute) {
    debugPrint('Basic observer - Pop: (${previousRoute?.settings.name}) <== (${route?.settings.name})');
    analyticsService.logEvent('view_screen', eventProperties: {'screen_name': previousRoute?.settings.name, 'type': 'pop'});
  }
}
